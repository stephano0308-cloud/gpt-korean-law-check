...
from .tax_case_review_v2 import review_related_laws, summarize_case

load_dotenv()

app = FastAPI(
    title="GPT Korean Law Check API - History Detail",
    version="1.1.0",
    description="조세사건 관련법령 검토를 위한 GPT용 FastAPI 서버 (연혁 세부 내역 포함)",
    servers=[
...
async def health() -> dict:
    return {
        "status": "ok",
        "service": "GPT Korean Law Check API - History Detail",
