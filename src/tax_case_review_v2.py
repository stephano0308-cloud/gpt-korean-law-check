...
from .history_tools_v2 import analyze_history_applicability
...
def summarize_case(document_text: str) -> Dict:
...
def _article_label(ref: Dict, law_detail: Dict, article: Optional[Dict]) -> Optional[str]:
    if article:
        title = article.get('조문제목') or '조문명 확인 필요'
        return f"{law_detail.get('법령명') or ref['law_name']} 제{ref.get('article')}조({title})"
    if ref.get('article'):
        return f"{law_detail.get('법령명') or ref['law_name']} 제{ref.get('article')}조(조문명 추가 확인 필요)"
    return law_detail.get('법령명') or ref['law_name']
...
def extract_addendum_analysis(law_detail: Dict, basis_dates: List[str]) -> Dict:
    ...
    for article in law_detail.get("조문", []):
        ...
        notes["detail_output"]["부칙존재여부"] = True
        notes["detail_output"]["부칙세부조문"].append({
            "조문번호": article.get("조문번호") or "부칙",
            "조문제목": article.get("조문제목") or None,
            "조문내용": article.get("조문내용") or None,
        })
        ...
        for sentence in sentences:
            ...
            if any(token in s for token in ["시행", "적용", "종전의 규정", "경과조치", "최초로"]):
                if "종전의 규정" in s or "경과조치" in s:
                    ...
                    if s not in notes["detail_output"]["경과조치"]:
                        notes["detail_output"]["경과조치"].append(s)
                elif "적용" in s or "최초로" in s:
                    if s not in notes["application_rules"]:
                        notes["application_rules"].append(s)
                    if s not in notes["detail_output"]["적용례"]:
                        notes["detail_output"]["적용례"].append(s)
                elif "시행" in s:
                    if s not in notes["effective_rules"]:
                        notes["effective_rules"].append(s)
                    if s not in notes["detail_output"]["시행일"]:
                        notes["detail_output"]["시행일"].append(s)
    ...
    for ref in overview["law_references"]:
        ...
        if not law_id:
            reviewed.append({
                ...
                "history_detail_output": history_analysis.get("history_detail_output"),
            })
            continue
        ...
        reviewed.append({
            ...
            "formatted_article_label": _article_label(ref, law_detail, article),
            "addendum_analysis": addendum_analysis,
            "addendum_detail_output": addendum_analysis.get("detail_output"),
            "history_analysis": history_analysis,
            "history_detail_output": history_analysis.get("history_detail_output"),
        })
