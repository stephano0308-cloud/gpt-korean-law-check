...
from .review_with_spelling import review_document_with_spelling
...
app = FastAPI(
    title="GPT Korean Law Check API - Spellcheck",
    version="1.1.0",
    description="조세사건 관련법령 검토 + 맞춤법/오탈자 검토를 위한 GPT용 FastAPI 서버",
    servers=[
...
class SpellcheckRequest(BaseModel):
    document_text: str = Field(..., description="맞춤법 검사를 수행할 문서 텍스트")


class FullReviewRequest(BaseModel):
    document_text: str = Field(..., description="관련법령 + 맞춤법 통합 검토 대상 문서 텍스트")
    include_law_text: bool = Field(False, description="조문 본문 포함 여부")
...
async def health() -> dict:
    return {
        "status": "ok",
        "service": "GPT Korean Law Check API - Spellcheck",
        "law_api_key": "configured" if os.environ.get("LAW_API_KEY") else "missing",
        "spell_checker": "configured" if KoreanSpellChecker().available else "missing_dependency",
    }
...
async def check_spelling_route(request: SpellcheckRequest) -> dict:
    checker = KoreanSpellChecker()
    return await asyncio.to_thread(checker.check_text, request.document_text)


@app.post("/review-document")
async def review_document_route(request: FullReviewRequest) -> dict:
    result = await asyncio.to_thread(review_document_with_spelling, request.document_text)
    if not request.include_law_text:
        for item in result.get("law_review", {}).get("reviewed_laws", []):
            item.pop("article_text", None)
    return result
